-- LOOTFORM Craft: roll MP/MAT/MDF/AGI as grade-only "option" stats
-- Date: 2026-08-31
--
-- Purpose:
--   Extend lootform_craft_atomic to also roll the 4 RPG-Maker-style
--   stats added in 202608310002 (mp/mat/mdf/agi_bonus_snapshot).
--
--   Confirmed with Management (chat, 2026-08-31): unlike the existing 6
--   stats, these 4 are NOT tuned per Design (every Design keeps base 0
--   for them). Instead each uses one shared constant, scaled by the same
--   per-grade stat_multiplier and rolled with the same per-grade
--   variance_percent already used for HP/ATK/DEF/LUCK/HEAL/VISION --
--   same mechanism, reusing game_grade_stat_multipliers as-is, no new
--   table.
--
--   Approved base constants (before grade multiplier / variance roll):
--     MP 5, MAT 3, MDF 3, AGI 3
--
--   Same signature, same security posture (service_role only) as the
--   function this replaces. Only the stat-roll section changes.
...
DECLARE
  ...
  -- Grade-only "option" base constants -- see migration header.
...
BEGIN
  ...
  -- Grade-only option stats: same multiplier/variance mechanism, but the
  -- base is a shared constant instead of a per-Design column.
