-- LOOTFORM Fix: re-introduced ROUND(double precision, integer) bug
-- Date: 2026-08-31
--
-- Purpose:
--   202608310003 (adding MP/MAT/MDF/AGI option-stat rolls) rewrote
--   lootform_craft_atomic from the PRE-fix version of the function body
--   (202608280001) instead of the already-fixed one (202608280002),
--   which lost the ::numeric cast that 202608280002 had specifically
--   added to make ROUND(..., 2) work. That reintroduced the exact same
--   bug: random() is double precision, so the variance-factor
--   subexpression resolves to double precision, and
--   ROUND(double precision, integer) does not exist in Postgres --
--   confirmed live via CRAFT ATOMIC RPC ERROR in the Next.js server log:
--   42883 "function round(double precision, integer) does not exist".
--   This broke every live Craft attempt (surfaced to players as
--   "Atomic Craft transaction ไม่พร้อม").
--
-- Fix: same as 202608280002 -- cast the variance-factor subexpression to
-- ::numeric before it multiplies into each running total, for all ten
-- stats (the original six plus the four new MP/MAT/MDF/AGI ones). No
-- behavior change beyond making the roll actually execute.
...
DECLARE
  ...
  c_mp_base CONSTANT integer := 5;
  c_mat_base CONSTANT integer := 3;
  c_mdf_base CONSTANT integer := 3;
  c_agi_base CONSTANT integer := 3;
BEGIN
  ...
  -- FIX: ::numeric cast before ROUND(), same as 202608280002 -- random()
  -- is double precision, and ROUND(double precision, 2) does not exist.
  ...
  v_mp_bonus := ROUND((c_mp_base * v_stat_multiplier
    * (1 + (random() * 2 - 1) * v_variance_percent / 100.0))::numeric);
  v_mat_bonus := ROUND((c_mat_base * v_stat_multiplier
    * (1 + (random() * 2 - 1) * v_variance_percent / 100.0))::numeric);
  v_mdf_bonus := ROUND((c_mdf_base * v_stat_multiplier
    * (1 + (random() * 2 - 1) * v_variance_percent / 100.0))::numeric);
  v_agi_bonus := ROUND((c_agi_base * v_stat_multiplier
    * (1 + (random() * 2 - 1) * v_variance_percent / 100.0))::numeric);
  ...
  v_response := jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'mp_bonus', v_item.mp_bonus_snapshot,
      'mat_bonus', v_item.mat_bonus_snapshot,
      'mdf_bonus', v_item.mdf_bonus_snapshot,
      'agi_bonus', v_item.agi_bonus_snapshot,
